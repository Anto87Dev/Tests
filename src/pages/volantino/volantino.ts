...
@IonicPage()
@Component({
  selector: 'page-volantino',
  templateUrl: 'volantino.html'
})
export class VolantinoPage {
  ...
  for(let cat in this.shared.getVolantino()){

    for(let prodotto in this.shared.getVolantino()[cat]){
  
      console.log(" this.shared.getVolantino(): ", this.shared.getVolantino()[cat][prodotto]);
  
      if(this.shared.getVolantino()[cat][prodotto].volantino){
  
        this.prodotti.push(this.shared.getVolantino()[cat][prodotto]);
